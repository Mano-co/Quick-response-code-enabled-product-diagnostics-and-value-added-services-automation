...
function renderDashboard() {
  ...
  const activeOrders = ordersCache.filter((o) => o.status !== "COMPLETED").length;
  ...
  live.forEach((o) => {
    ...
    div.innerHTML = `
      ...
      <div class="status-btns">
        ${nextStep ? `<button class="btn btn-primary btn-sm" onclick="advanceOrder('${o.id}')">Mark ${nextStep}</button>` : ""}
      </div>
    `;
    box.appendChild(div);
  });
...
async function advanceOrder(orderId) {
  ...
  if (!next) return;
  await dbSet("orders", orderId, { status: next });
  toast(`${orderId} → ${next}`);
}
