...
function listenOrderStatus() {
  if (!state.orderId) {
    document.getElementById("orderStatusList").innerHTML = `<p class="muted">No active order yet.</p>`;
    return;
  }
...
function renderOrderStatus(order) {
  ...
  list.innerHTML = `
    <div class="row"><span class="muted">Order</span><b class="mono">${order.id}</b></div>
    <div class="row"><span class="muted">Table</span><b>${order.tableNumber}</b></div>
    <div class="row"><span class="muted">Total</span><b class="mono">₹${order.total}</b></div>
    ${(order.items || []).map((i) => `<div class="row"><span>${i.name} × ${i.qty}</span><span class="mono">₹${i.price * i.qty}</span></div>`).join("")}
    ${order.status === "SERVED" ? `<button class="btn btn-primary" onclick="go('bill')">View Bill</button>` : ""}
  `;
}
